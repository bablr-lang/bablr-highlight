  ...
  while (el) {
    if (el.getAttribute('type') === 'LeftOffset') {
      ...
      el =
        ...
        (el.parentElement.tagName === 'SPAN' ? el.parentElement : null);
    }
  }
...
export const highlightCode = (el) => {
    ...
    switch (tag.type) {
      ...
      case CloseNodeTag: {
        if (stack.size > 1) {
          ...
          span.setAttribute('type', open.value.type.description);
          if (names) {
            span.setAttribute('class', names);
          }
    ...
    set(value) {
      ...
      if (store._selectedRange) {
        ...
        if (prevParent) {
          ...
          prevAnchor.removeChild(
            [...prevAnchor.childNodes].find((child) => child.classList.contains('tooltip')),
          );
        }
      }
  ...
  Object.defineProperty(store, 'hoverTarget', {
    set(value) {
      ...
      if (store._hoverTarget) {
        ...
        prevEl.removeChild(
          [...prevEl.childNodes].find((child) => child.classList.contains('hover')),
        );
      }
  ...
  el.addEventListener('mouseover', (e) => {
    if (store.selectionState === 'selecting') {
      ...
      if (e.fromElement?.tagName !== 'SPAN' && store.outTimeout) {
