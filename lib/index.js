...
export const highlightCode = (el, language, matcher = language.defaultMatcher) => {
  let tags = streamParse(language, matcher, el.innerText)[Symbol.iterator]();
...
export const highlightAll = (languages) => {
  ...
  for (let block of codeBlocks) {
    let canonicalURL = block.getAttribute('bablr-language');
    let language = languages.get(canonicalURL);
    let flagsStr = block.getAttribute('bablr-production-flags');

    let type = block.getAttribute('bablr-production');
      ...
      type
        ? buildEmbeddedMatcher(
            buildPropertyMatcher(
              null,
              null,
              buildBasicNodeMatcher(buildOpenNodeMatcher(buildNodeFlags(), type)),
            ),
          )
